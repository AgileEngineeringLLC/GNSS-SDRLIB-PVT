...
extern void sendrtcmobs(obsd_t *obsd, sdrsoc_t *soc, int nsat)
{
    ...
    /* QZSS rtcm msm */ // Must leave this code enabled for RTCM to work
    ...
    /* GLONASS rtcm msm */ // Must leave this code enabled for RTCM to work
    gen_rtcm3(&rtcm,1087,1);
    if (send(soc->c_soc,(char*)rtcm.buff,rtcm.nbyte,0)==SOCKET_ERROR) {
        soc->flag=OFF;
    }
    /* BeiDou rtcm msm */ // Must leave this code enabled for RTCM to work
