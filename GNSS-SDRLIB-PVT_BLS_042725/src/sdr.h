...
#include <SDL2/SDL.h> // for SDL2 gui
#include <SDL2/SDL_ttf.h> // for SDL2 gui
#include <pthread.h>
...
#define LOW_PR        60e-3 * CTIME
#define HIGH_PR       92e-3 * CTIME

// SNR thresholds
#define SNR_RESET_THRES		16  	// Threshold to reset sdrch channel
#define SNR_PVT_THRES 		20      // Threshold to use obs for PVT
...
// SDL2 GUI
#define SCREEN_LOC_X  100
#define SCREEN_LOC_Y  100
#define SCREEN_WIDTH  1350 // 1280
#define SCREEN_HEIGHT 800  // 720
#define FONT_SIZE     28 // 24 default
...
typedef struct {
        ...
        int nsat;
        int satList[MAXSAT];
        double obs_v[10*MAXSAT]; // Array to hold all obs data
        double vk1_v[MAXSAT]; // Array to hold all obs data
        double clkBias;
        double xyzdt[4];
        //double elapsedTime;
        int pvtflag;
...
typedef struct {
        ...
        int resetflag;   // DK added, channel reset flag
        double elapsed_time; // DK added, elapsed sdrch time
} sdrch_t;
...
extern thread_t hguithread;   /* GUI thread */
...
//extern int calculateGDOP(double *G_v, double lat, double lon, int numSat,
//                   double *gdop);
...
// sdrekf.c -------------------------------------------------------------------
extern int ekfFilter(double *xs_v, double *pr_v, int numSat,
                 double xyzdt_v[], double *gdop);
extern int ekfInit(void);
extern int bancroft(double *obs_v, double *pos_v, int nsat);
extern int lorentz(double *x, double *y, double *p);
